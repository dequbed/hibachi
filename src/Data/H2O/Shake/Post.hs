...
addPostBuildRule = addBuiltinRule noLint noIdentity run
  where
    ...
    run key oldBin mode = do
        ...
        let a = act (fromPostQ key)
        if mode == RunDependenciesSame && Just ver == (version <$> old) && Just (postOid key) == (blobOid <$> old) then
            return $ RunResult ChangedNothing (fromJust oldBin) $ PostR (path $ fromJust old)
        else do
            path <- a
            let new = putEx $ PostA (postOid key) ver path
...
getEx :: B.ByteString -> PostA
getEx = undefined
putEx :: PostA -> B.ByteString
putEx = undefined
